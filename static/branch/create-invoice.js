...
function udpdateData(id){
    for(var n = 0; n < data.length; n++) {
        if(data[n].id == id) {
            data[n].quantity = parseInt(data[n].quantity) + parseInt($('input[name=quantity]').val())
            viewTable()
        }
    }
    return false;
...
function viewTable (){
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        row.append($("<td>" + data[i].barcode + "</td>"));
    ...
    .append('<tr>').append('<td style="font-weight: bold;" colspan="4">Total:</td>')
